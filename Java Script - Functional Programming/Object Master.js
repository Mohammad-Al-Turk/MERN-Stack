...
constbListPkmn=pokémon.filter(p=>p==="B");
console.log(constbListPkmn)

constpkmnIds=pokémon.map(p=>p.id)
console.log(constpkmnIds)



divisibleBy3 = pokémon.filter(p => p.id % 3 === 0);
...
const poisonOnlyNames = pokémon.filter(p => p.types.length === 1 && p.types[0] === "poison").map(p => p.name);
console.log(poisonOnlyNames);

const firstTypeWhenSecondIsFlying = pokémon.filter(p => p.types[1] === "flying").map(p => p.types[0]);
console.log(firstTypeWhenSecondIsFlying);
